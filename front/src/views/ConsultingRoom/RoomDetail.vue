<template>
    ...
    <v-main class=" " >
      <v-row class="mx-10 my-10" v-if="this.consulting_room.data" justify="center">
        ...
        <v-col cols="12" md="6">
            <v-img class="mx-auto my-10" :src="this.consulting_room.data.qr_code" width="200"></v-img>
            <v-row justify="center">
              <v-col> 
                <PxCreate></PxCreate>
                </v-col>
                <v-col>
                  <v-btn  class="text-none" :href="'/dashboard/consultingroom/patients-all/'+this.consulting_room.data.id">
                        Ver lista de Px
                    </v-btn>
                </v-col>
                <v-col> 
                    <v-btn class="text-none" :href="'/appointment/gen/'+this.consulting_room.data.id">
                        Agendar consulta
                    </v-btn>
                </v-col>
                <v-col>
      ...
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            color="blue darken-1"
            text
            @click="CreateServiceDialog = false"
          >
            Cancelar
          </v-btn>
          <v-btn
...
export default {
    ...
    payment_headers: [
          ...
          { text: 'Servicio realizado', value: 'treatment_service_name', sortable: false },
        ],
    headers: [
          { text: 'ID', value: 'id' },
    ...
    methods:{
        ...
        async init(){
            this.consulting_room = await this.getConsultingRoom(this.$route.params.id)
            this.appointments =  await this.getAppointmentsPerConsultingRoom(this.$route.params.id)
            this.services = await this.getServicesConsultingRoom(this.$route.params.id)
            this.payments = await this.getPaymentsPerConsultingRoom(this.$route.params.id)
        },
       async submit(){
            try {
